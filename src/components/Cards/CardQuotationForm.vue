          ...
          <div class="w-full lg:w-6/12 px-4">
            <div class="relative w-full mb-3">
              ...
              >
                ...
                <span class="mx-4 text-xs font-semibold inline-block py-1 px-2 rounded text-emerald-600 bg-emerald-200 last:mr-0 mr-1 uppercase" :class="!thaiDateFormattedDue ? 'hidden' : ''">
...
const createNewDocument = async () => {
  ...
  try{
    const response = await axios.post(`${process.env.VUE_APP_API_BACKEND}/quotation/CreateVat`,
        formData.value,
      {
        headers: {
          'auth-token' : `${process.env.VUE_APP_AUTH_TOKEN_ADMIN}`

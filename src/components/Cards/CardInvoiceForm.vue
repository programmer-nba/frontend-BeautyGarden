    ...
    <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
      <form>
          ...
          <div class="w-full lg:w-6/12 px-4">
            <div class="relative w-full mb-3">
              ...
              <input
                ...
                disabled
                :value="predictNextCode(invoice_lastcode)"
              />
            </div>
          </div>
          <div class="w-full lg:w-6/12 px-4">
        ...
        <div class="flex w-full justify-center items-center mt-4">
          ...
          :invoice="{
            code: predictNextCode(invoice_lastcode) || '-',
            start_date: thaiDate || '-',
            refer: formData.quotation,
          }"
...
import DraftInvoice from '@/components/Modals/DraftInvoice.vue'

const edit = ref(true)
const thaiDate = ref('')
const invoice_lastcode = ref('')
...
const predictNextCode = (curCode) => {
  if(curCode){
    ...
    let newCode = 
      ...
      : 'INV' + formattedDate + incrementedNumericPart
...
const createNewDocument = async () => {
  ...
  try{
    const response = await axios.post(`${process.env.VUE_APP_API_BACKEND}/invoice/PrintInviuceVat`,
        formData.value,
      {
        headers: {
          'auth-token' : `${process.env.VUE_APP_AUTH_TOKEN_ADMIN}`
...
const fetchData = async () => {
  try{
    const response = await axios.get(`${process.env.VUE_APP_API_BACKEND}/invoice/getInvoiceVatAll`,
      {
        headers: {
          'auth-token' : `${process.env.VUE_APP_AUTH_TOKEN_ADMIN}`
        }
    ...
    if(response.data.status){
      invoice_lastcode.value = response.data.data[response.data.data.length - 1]?.receipt
    } else {
      invoice_lastcode.value = null
    }
  }
  catch(err){
    invoice_lastcode.value = null

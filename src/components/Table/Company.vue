      ...
      <div class="text-center flex justify-between">
        <div class="flex flex-col justify-center gap-y-2 items-center">
            <div class="flex flex-wrap w-full gap-3 items-center">
                <Dropdown v-model="logo" :options="logos" optionLabel="" placeholder="เลือกโลโก้" class="w-36">
                    <template #value="slotProps">
                        <div v-if="slotProps.value" class="flex align-items-center">
                            <img :alt="slotProps.value?._id" :src="`https://drive.google.com/thumbnail?id=${slotProps.value?.profile_image}`" :class="`mr-2 flag`" style="width: 75px" />
                        </div>
                    </template>
                    <template #option="slotProps">
                        <div class="flex align-items-center">
                            <img :alt="slotProps.option?._id" :src="`https://drive.google.com/thumbnail?id=${slotProps.option?.profile_image}`" :class="`mr-2 flag`" style="width: 75px" />
                        </div>
                    </template>
                </Dropdown>
                <Dropdown 
                v-if="!newData"
                @change="getBanks" v-model="headData" :options="headDatas"
                ...
                <div class="flex gap-2 items-center border-l pl-3">
                    <Image v-if="logo" :src="`https://drive.google.com/thumbnail?id=${logo?.profile_image}`" class="rounded-full" alt="Image" width="75" preview />
                    <h6 class="text-blueGray-700 text-xl font-bold">
                      {{ headData.Branch_company_name }}
                      {{ headData.Branch_iden ? `(${headData.Branch_iden})` : "" }}
                      <span :class="headData.isVat ? 'px-2 mx-2 rounded-xl text-sm bg-yellow-300 text-orange-600' : ''">{{ headData.isVat ? 'VAT' : '' }}</span>
                    </h6>
                </div>
            </div>
    ...
    <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
      <form>
          ...
          <div class="w-full lg:w-6/12 px-4">
            <div class="relative w-full mb-3">
                <div class="flex gap-2">
                    ...
                    <InputSwitch v-if="edit || newData" @change="changeBranch" v-model="isMain" />
                </div>
              <input
                type="text"
                class="px-3 py-3 placeholder-blueGray-300 text-blueGray-600 rounded text-md shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
        ...
        <div class="flex flex-col">
            ...
            <div v-if="edit || newData" class="flex items-center justify-center mt-3 border-t pt-3">
                <div class="w-full lg:w-6/12 px-4">
                    <div class="relative w-full mb-3">
                      <div class="flex gap-2 items-center mb-2">
                          <span class="block uppercase text-blueGray-600 text-md font-bold text-green-500">
                              ชื่อบัญชี
                          </span>
                      </div>
                      <input
                ...
                <Button label="เพิ่ม" @click="addBank" class="px-2 pt-3 m-0 rounded-md text-green-500" />
            </div>
          </div>
        ...
        <div v-if="headData.Branch_company_name" class="flex gap-4">
          ...
          >
            ...
            <Button v-if="edit" :loading="loading" @click="toDeleteSign($event, signature._id)" label="ลบ" outlined class="text-red-500 hover:bg-red-200"></Button>
          </div>
          <Button v-if="edit" size="small" icon="pi pi-plus" class="hover:bg-white rounded-full mt-1 border-green-500 border-2 px-5 ml-5 w-[70px] h-[70px]" @click="openAddSign=true"/>
        </div>
      </form>
    </div>
  </div>

  <Dialog v-model:visible="openAddSign" modal header="เพิ่มลายเซ็นใหม่" :style="{ width: '25rem' }">
    <div class="flex align-items-center gap-3 mb-3">
        <label for="name" class="font-semibold w-6rem">ชื่อ : </label>
        <InputText id="name" class="flex-auto px-2 ml-2 border-b" autocomplete="off" v-model="signature.name" />
    </div>
    <div class="flex align-items-center gap-3 mb-5">
        <label for="position" class="font-semibold w-6rem">ตำแหน่ง : </label>
        <InputText id="position" class="flex-auto px-2 ml-2 border-b" autocomplete="off" v-model="signature.position" />
    </div>
    <div>
        <div class="w-full flex justify-end">
            <Button v-if="previewSign" size="small" icon="pi pi-times" class="text-red-500 relative hover:font-bold -bottom-16" @click="previewSign=null" />
        </div>
    ...
    <div class="flex justify-center gap-2 mt-5">
        <Button :loading="loading" label="ยกเลิก" @click="openAddSign = false" class="px-2 text-red-400 hover:bg-red-200"/>
        <Button :loading="loading" label="บันทึก" @click="addSign" class="px-2 text-green-700 hover:bg-green-200"/>
    </div>
</Dialog>
</template>
<script setup>
...
const addSign = async () => {
    ...
    try {
        ...
        if (response.data) {
            ...
            toast.add({ severity: 'success', summary: 'สำเร็จ', detail: 'คุณเพิ่มลายเซ็นใหม่แล้ว', group: 'br', life: 3000 });
            loading.value = false
        ...
        loading.value = false
        console.log(err)
        toast.add({ severity: 'error', summary: 'เกิดข้อผิดพลาด', detail: 'เพิ่มลายเซ็นใหม่ล้มเหลว', life: 3000 });
    }
}
...
const deleteSign = async (id) => {
    ...
    try {
        ...
        if (response.data) {
            console.log(response.data)
            toast.add({ severity: 'success', summary: 'สำเร็จ', detail: 'ลบลายเซ็นแล้ว', group: 'br', life: 3000 });
            loading.value = false
            openAddSign.value = false
            signature.value = {}
            getSignatures()
        ...
        loading.value = false
        console.log(err)
        toast.add({ severity: 'error', summary: 'เกิดข้อผิดพลาด', detail: 'ลบลายเซ็นล้มเหลว', life: 3000 });
    }
}
  ...
  await axios
    ...
    .then(async (response) => {
        if(response.data){
            ...
            toast.add({ severity: 'success', summary: 'สำเร็จ', detail: 'คุณแก้ไขข้อมูลสำเร็จ', group: 'br', life: 3000 });
            edit.value = false
            getHeaders()
        }
    ...
    .catch((err) => {
      loading.value = false
      console.log(err);
      toast.add({ severity: 'error', summary: 'เกิดข้อผิดพลาด', detail: 'แก้ไขข้อมูลไม่สำเร็จ', group: 'br', life: 3000 });
      edit.value = false
      getHeaders()
    });
}
  ...
  await axios
    ...
    .then((response) => {
        if(response.data){
            loading.value = false
            console.log(response.data.message);
            toast.add({ severity: 'success', summary: 'สำเร็จ', detail: 'เพิ่มข้อมูลสำเร็จ', group: 'br', life: 3000 });
            edit.value = false
            getHeaders()
        }
    ...
    .catch((err) => {
      loading.value = false
      console.log(err);
      toast.add({ severity: 'error', summary: 'เกิดข้อผิดพลาด', detail: 'เพิ่มมูลไม่สำเร็จ', group: 'br', life: 3000 });
      edit.value = false
      getHeaders()
    });
};
...
const deleteCurCompany = async (id) => {
    ...
    try {
        ...
        if(response){
            ...
            toast.add({ severity: 'success', summary: 'สำเร็จ', detail: 'ลบข้อมูลแล้ว', group: 'br', life: 3000 });
            loading.value = false
            getHeaders().then(()=> {
                headData.value = headDatas.value[0]
                banks.value = headDatas.value[0].bank
        ...
        conole.log(err)
        toast.add({ severity: 'error', summary: 'เกิดข้อผิดพลาด', detail: 'ลบข้อมูลไม่สำเร็จ', group: 'br', life: 3000 });
        loading.value = false
    }
}
</script>

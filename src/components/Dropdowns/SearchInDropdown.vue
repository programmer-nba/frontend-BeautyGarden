          ...
          <div ref="popoverDropdownRef" class="bg-emerald-500 text-base z-50 float-left py-2 list-none text-left rounded shadow-lg mt-1 min-w-48" v-bind:class="{hidden: !dropdownPopoverShow,block: dropdownPopoverShow,}">
            <a class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-white">
                <div class="relative flex w-full flex-wrap items-stretch mb-3">
                    <input v-model="searchTarget" type="text" placeholder="IV2024MMDD" class="px-2 py-1 placeholder-blueGray-300 text-blueGray-600 relative bg-white rounded text-sm shadow outline-none focus:outline-none focus:shadow-outline w-full pr-10"/>
            ...
            await axios.get(`${process.env.VUE_APP_API_BACKEND}/invoice/getIVAllfilter`,
                {
                    ...
                    if(response.data.status){
                        ...
                        this.originalInvoicesCode = response.data.data.reverse().map((item)=>{
                            return item.invoice
                        })
                        this.filterIn()
                    }
